...
RODADA AUTORIA (Crédito da Desenvolvedora):
- Adicionado crédito "Desenvolvido por Lana Gleizi Vieira Paes" no rodapé
  do PDF executivo, logo após a nota metodológica, com formatação discreta
  mas legível (fonte menor, cor cinza-escuro).
- Mesmo crédito adicionado na aba "Resumo_Mensal" do Excel e como linha
  final do CSV, garantindo autoria em todos os artefatos exportados.

RODADA DECIMAL (Consistência com transaction_parser/income_calculator):
- Migração de float para decimal.Decimal em format_currency(), variáveis
...
# ---------------------------------------------------------------------------
# RODADA AUTORIA: crédito da desenvolvedora (visível em PDF, Excel e CSV)
# ---------------------------------------------------------------------------
AUTORIA_CREDITO = "Desenvolvido por Lana Gleizi Vieira Paes"
...
def _canary_negative_valid(tx) -> None:
    ...
    """
    if _to_decimal(tx.amount) < 0:
        logger.error(
            "INCONSISTÊNCIA NO RELATÓRIO: entrada válida com valor negativo "
            "(%s | %s | %s | %s). Regressão de parser/calculator suspeita.",
            getattr(tx, "date", "?"),
            tx.description,
            tx.amount,
...
def _build_cell_styles() -> Dict[str, ParagraphStyle]:
    ...
    return {
        ...
        "note": ParagraphStyle("Note", parent=base["Normal"], fontSize=8,
                               textColor=GRAY_TEXT),
        # RODADA AUTORIA: estilo próprio para o crédito da desenvolvedora
        "autoria": ParagraphStyle(
            "Autoria", parent=base["Normal"],
            fontSize=8.5, textColor=GRAY_TEXT, alignment=TA_CENTER,
...
def generate_report(metrics: Dict[str, Any], holder_name: str,
    ...
    kpi_data = [
        ...
        [Paragraph(f"<b>{format_currency(metrics.get('total_geral', Decimal('0.00')))}</b>", kpi_value_style),
         Paragraph(f"<b>{format_currency(metrics.get('media_mensal_geral', Decimal('0.00')))}</b>", kpi_value_style),
         Paragraph(f"<b>{format_currency(metrics.get('media_meses_completos', Decimal('0.00')))}</b>", kpi_value_style)],
    ]
    ...
    # RODADA DECIMAL: acumuladores em Decimal para precisão monetária
    total_geral_check = Decimal('0.00')
    total_qtd_check = 0
    for item in metrics.get("resumo_mensal", []):
        ...
        total_geral_check += _to_decimal(item["total_valido"])
    ...
    resumo_data.append([
        ...
        Paragraph(f"<b>{format_currency(metrics.get('total_geral', Decimal('0.00')))}</b>", S["tot_r"]),
    ])
    ...
    if valid_txs:
        ...
        for year, month in sorted_months:
            ...
            # RODADA DECIMAL: subtotal em Decimal
            ...
            for tx in txs_do_mes:
                ...
                subtotal += _to_decimal(tx.amount)
    ...
    # --- RODADA AUTORIA: crédito da desenvolvedora no rodapé do PDF ---
    story.append(Spacer(1, 0.3 * cm))
    story.append(Paragraph(AUTORIA_CREDITO, S["autoria"]))
...
def generate_excel(metrics: Dict[str, Any], holder_name: str = "",
    ...
    for item in metrics.get("resumo_mensal", []):
        ws1.append([item["month_label"], item.get("dias_cobertos", 0),
                    ...
                    float(_to_decimal(item["total_valido"]).quantize(Decimal('0.01')))])
    ws1.append(["TOTAL", "",
                sum(i["qtd_entradas_validas"] for i in metrics.get("resumo_mensal", [])),
                float(_to_decimal(metrics.get("total_geral", Decimal('0.00'))).quantize(Decimal('0.01')))])
    ws1.append([])
    ws1.append(["Titular", holder_name or "Não informado"])
    ws1.append(["Instituições", ", ".join(institutions or []) or "Não identificadas"])
    ws1.append(["Total Geral Acumulado",
                float(_to_decimal(metrics.get("total_geral", Decimal('0.00'))).quantize(Decimal('0.01')))])
    ws1.append(["Média Mensal Geral",
                float(_to_decimal(metrics.get("media_mensal_geral", Decimal('0.00'))).quantize(Decimal('0.01')))])
    ws1.append(["Média Meses Completos",
                float(_to_decimal(metrics.get("media_meses_completos", Decimal('0.00'))).quantize(Decimal('0.01')))])
    ...
    # RODADA AUTORIA: crédito da desenvolvedora na aba Resumo do Excel
    ws1.append([])
    ws1.append([AUTORIA_CREDITO, "", "", ""])
    ...
    for tx in metrics.get("entradas_validas", []):
        ...
        ws2.append([format_date(tx.date), tx.description,
                    float(_to_decimal(tx.amount).quantize(Decimal('0.01'))),
                    getattr(tx, "bank", ""), obs])
    if manual_any:
        ws2.append([])
    ...
    for item in metrics.get("entradas_excluidas", []):
        ws3.append([format_date(item["date"]), item["description"], item["reason"],
                    float(_to_decimal(item["amount"]).quantize(Decimal('0.01')))])
...
def generate_csv(metrics: Dict[str, Any]) -> bytes:
    ...
    for item in metrics.get("resumo_mensal", []):
        w.writerow([item["month_label"], item.get("dias_cobertos", 0),
                    item["qtd_entradas_validas"],
                    f"{_to_decimal(item['total_valido']):.2f}".replace(".", ",")])
    w.writerow(["TOTAL", "",
                sum(i["qtd_entradas_validas"] for i in metrics.get("resumo_mensal", [])),
                f"{_to_decimal(metrics.get('total_geral', Decimal('0.00'))):.2f}".replace(".", ",")])
    ...
    for tx in metrics.get("entradas_validas", []):
        ...
        w.writerow([format_date(tx.date), tx.description,
                    f"{_to_decimal(tx.amount):.2f}".replace(".", ","),
                    getattr(tx, "bank", ""), obs])
    if manual_any:
        w.writerow([])
    ...
    for item in metrics.get("entradas_excluidas", []):
        w.writerow([format_date(item["date"]), item["description"], item["reason"],
                    f"{_to_decimal(item['amount']):.2f}".replace(".", ",")])

    # RODADA AUTORIA: crédito da desenvolvedora como linha final do CSV
    w.writerow([])
    w.writerow([AUTORIA_CREDITO])
